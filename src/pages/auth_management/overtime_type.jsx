 ...
 * @LastEditTime: 2024-12-02 14:24:17
 */
...
import { reqGetOvertimeTypeDatas, reqAddOvertimeTypeDatas, reqDeleteOvertimeTypeDatas } from '../../api/index'
const ServiceLine = () => {
  ...
  const handDelete = async (e) => {
    const result = await reqDeleteOvertimeTypeDatas(e)
    if(result.status === 1){
      getTableData()
      message.info('删除成功...')
    }else{
  ...
  const handleOk = () => {
    ...
    const result = await reqAddOvertimeTypeDatas(val)
    if(result.status === 1){
